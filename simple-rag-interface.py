...
class VuetifyRAG:
    ...
    def __init__(self, chroma_db_path: str = "./chroma_vuetify_db"):
    ...
    def _setup_chromadb(self):
        ...
        try:
            ...
            print("Run setup_chromadb.py first!")
            exit(1)
...
Please provide a helpful answer based on the documentation context above."""
    ...
    def _format_simple_response(self, search_results: List[Dict[str, Any]]) -> str:
        ...
        for i, result in enumerate(search_results, 1):
            ...
            response_parts.append(f"{i}. From {component} - {section} (relevance: {score:.2f}):")
        ...
        try:
            ...
            for i, source in enumerate(result['sources'], 1):
                print(f"  {i}. {source['component']} - {source['section']} "
                      f"({source['type']}, similarity: {source['similarity']})")
...
def batch_test_mode(rag: VuetifyRAG):
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        for source in result['sources'][:2]:  # Show top 2 sources
            print(f"  - {source['component']} ({source['similarity']})")
